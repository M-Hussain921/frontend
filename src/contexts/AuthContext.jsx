...
export const AuthProvider = ({ children }) => {
  ...
  const setAuth = useCallback(
    (newToken, userData) => {
      ...
      if (userData) {
        ...
        // If only token provided, fetch profile
        fetchProfile();
      }
    },
    [fetchProfile],
  ...
  const socialLoginHandler = useCallback((accessToken, userData) => {
  ...
  const register = async (userData, role = "user") => {
    try {
      const endpoint =
        role === "delivery" ? "/auth/delivery/register" : "/auth/user/register";
      ...
      if (accessToken) {
        ...
        try{
 const profileRes = await getProfile();
 const userDataFromProfile = profileRes.user || profileRes.data?.user;
  if (userDataFromProfile) {
          setUser(userDataFromProfile);
          setIsAuthenticated(true);
          return { success: true, user: userDataFromProfile };
        }
        } catch(err){
console.warn("Profile fetch failed after registration, using response.user", err);
        }
